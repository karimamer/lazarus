  ...
  (call/cc
    (lambda (escape)
      (with-handlers ([exn:fail? (lambda (condition)
                                  ...
                                  (displayln "Attempting simple auto-fix...")
...
(define (extract-variable-name error-msg)
  (define match (regexp-match #rx"variable.*'([^']*)" error-msg))
  (if match
      (second match)
      #f))
...
(define (make-http-call endpoint body)
  ...
  (define is-claude (string-contains? endpoint "anthropic"))
  (define headers (if is-claude
...
(define (split-url url)
  ...
  (values (symbol->string (url-scheme uri))
          (url-host uri)
          (url-port uri)
